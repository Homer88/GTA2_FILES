...
struct S82_BaseCar {
    ...
    // === Блок 4: Управление и таймеры (0x30 - 0x3F) ===
    ...
    f32 DragCoefficient;    // 0x48 - Коэффициент сопротивления воздуха

    // === Блок 6: Дополнительные параметры (0x4C - 0x67) ===
    u32 DamageFlags;        // 0x4C - Флаги повреждений
    u32 LightState;         // 0x50 - Состояние освещения (фары, поворотники)
    u8 EngineOn;            // 0x54 - Двигатель заведён (0/1)
    u8 Handbrake;           // 0x55 - Ручной тормоз (0/1)
    u8 Reserved1;           // 0x56 - Выравнивание
    u8 Reserved2;           // 0x57 - Выравнивание
    f32 SuspensionHeight;   // 0x58 - Высота подвески
    u32 WheelRotation[2];   // 0x5C - Вращение колёс [передние, задние]
